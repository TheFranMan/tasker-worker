...
func Service2UpdateUser(app *application.App, request repo.Request, id int) error {
	...
	if nil != err {
		...
		return app.Repo.MarkJobFailed(id, fmt.Errorf("cannot update user email from service 2: %w", err))
	}
